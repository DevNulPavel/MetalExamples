...
quaternion_float AAPL_SIMD_OVERLOAD quaternion_from_matrix3x3(matrix_float3x3 m)
{
    ...
    float diagonal =  m.columns[0][0] + m.columns[1][1] + m.columns[2][2];

    if(diagonal > 0)
    {
        diagonal = sqrt(diagonal+1.0) * 2.0;
        ...
        diagonal = sqrt(1.0 + m.columns[0][0] - m.columns[1][1] -  m.columns[2][2]) / 2.0;
        ...
        diagonal = sqrt(1.0 + m.columns[1][1] - m.columns[0][0] - m.columns[2][2]) / 2.0;
        ...
        diagonal = sqrt(1.0 + m.columns[2][2] - m.columns[0][0] - m.columns[1][1]) / 2.0;
