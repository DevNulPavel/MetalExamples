...
// Общий хедер с общими типами
#import "AAPLShaderTypes.h"

// Коэффициенты конвертации для перехода к черно-белому
constant half3 kRec709Luma = half3(0.2126, 0.7152, 0.0722);

// Функция вычисления
kernel void grayscaleKernel(texture2d<half, access::read>  inTexture  [[texture(AAPLTextureIndexInput)]],
                            texture2d<half, access::write> outTexture [[texture(AAPLTextureIndexOutput)]],
                            uint2                          gid         [[thread_position_in_grid]]) {   // id группы вычисления
    // Проверяем, что ид группы попадает под размер текстуры
